    ...
    case lists:keymember(Name, #ethif.name, IFS) of
        ...
        false ->
            try StartLinkFn() of
                ...
                Else ->
                    {reply, {error, {if_start_failed, Else}}, State}
            catch
                Type:Error ->
                    {reply, {error, {if_start_failed, {Type, Error}}}, State}
            end
    end;
        ...
        #ethif{name=If, pid=Pid} ->
            try AttachFn(Pid) of
                R = {ok, _Pid} ->
                    {reply, R, State};
                Else ->
                    {reply, {error, {attach_start_failed, Else}}, State}
            catch
                Type:Error ->
                    {reply, {error, {attach_start_failed, {Type, Error}}}, State}
            end;
        false ->
